...
* Switching from the VDD reference to an internal one costs a 40 µs settling (Table 32-4; 20 µs between two internal references). The core runs the ADC in Low Latency mode, where the analog modules are reconfigured as soon as CTRLC is written and the settling is reported by the ADCBUSY bit in `ADC0.STATUS`; a conversion started before it clears runs against an unsettled reference and returns full scale. `vddRead()` therefore polls ADCBUSY before starting, and again after restoring the previous reference so that an `analogRead()` immediately after it never converts while the ADC is still settling. The settling dominates the call: about 55 µs in total at 24 MHz, blocking.
