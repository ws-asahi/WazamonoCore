...
* Switching from the VDD reference to an internal one costs a 40 µs settling (Table 32-4; 20 µs between two internal references). The ADC inserts that settling itself at the start of the next conversion, timed in units of `CLKCTRL.MCLKTIMEBASE`, which the core sets in `init_ADC0()` (32.3.1 step 1) - with that register at its reset value the settling collapses and internal-reference conversions return 0 or 1023, which is what the first release of WazamonoCore did. `vddRead()` also checks the ADCBUSY bit in `ADC0.STATUS` before starting and after restoring the previous reference, the datasheet's documented way to see settling in progress; on the parts measured it never has to wait. The settling dominates the call: about 55 µs in total at 24 MHz, blocking.
