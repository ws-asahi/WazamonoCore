...
>  
> **SPI クロックの注意:** AVR DU の SPI 分周は /2, /4, /8 ... のため、24 MHz 動作では `SPISettings(14000000)` の要求が **12 MHz** に丸められ、Uno R3(8 MHz)より速くなります。  
> W5100 系イーサネットシールドはこの速度に追従できないため、同梱の **Ethernet ライブラリ**は SPI クロック要求を 8 MHz に変更してあります(12/16/20/24 MHz のいずれでも動作)。  
> Uno R3 用イーサネットシールドは直挿しで CS = D10(W5100) / D4(SD カード) のまま使えます。D13(SCK) は Serial2 の TX と共用のため、Ethernet/SD 使用中は Serial2 を開かないでください。  
> 他の SPI デバイスでも、Uno 向けライブラリが 14 MHz 前後を要求している場合は同様の問題が起こり得ます。  
>  
