...
AVR DU の OSCHF は USB の SOF を基準に自らの周波数を補正する機能(オートチューン)を持ち、  
コアは USB 初期化時にこれを有効にしています(Incremental 算法)。他のモダン AVR には無い DU 固有の機能です。  
Tachi と Kunai で、外部基準に対して周波数を連続測定した結果をまとめます。  

| 状態 | 周波数の精度(実測) |
|------|---------------------|
| USB 接続中 | **±0.12% 以内**。温度が変わっても追従して維持 |
| USB 接続後に切断 | 切断時点の補正値を保持。以後は温度に応じてずれる |
| USB 未接続| 工場校正のみ。実測 −0.44〜+0.08%(データシート上の保証は ±2%) |

- **運用中も追従します。** 補正は列挙時だけでなく、USB 接続中は常に働きます。  
周波数が約 ±0.1% の不感帯を外れると 1 段ずつ戻します。  
ダイ温度センサーの読みで最大約 15 °C の幅の温度変化を与えた時、周波数は ±0.12% 以内に保たれました(USB フルスピードの許容は ±0.25%)。  
- **補正は段差で行われます。** 1 段は約 0.07%(上げ)/約 0.04%(下げ)で、USB 接続中はこの大きさの周波数の段差が随時起こり得ます。  
`millis()` や UART には影響しない大きさですが、周波数の短期安定性そのものが必要な用途では考慮してください。
- **USB 非接続時は追従しません。** 内蔵発振器は温度が上がるほど速くなり、その変化は約 +0.015〜+0.035%/°C でした。
UART の許容(概ね ±2%)に対しては十分な精度ですが、周波数の絶対精度が要る用途には向きません。
