...
- USB 用の 48 MHz(CLK_USB)は内蔵 PLL48M が生成します。USB 接続中は OSCHF が USB の SOF(ホストの水晶で刻まれる 1 ms 周期の信号)に自動追従するため、水晶なしでも USB は仕様通りに機能します(下記「オートチューンの実測」参照)。

---

### オートチューンの実測

AVR DU の OSCHF は USB の SOF を基準に自らの周波数を補正する機能(オートチューン)を持ち、コアは USB 初期化時にこれを有効にしています(Incremental 算法)。他のモダン AVR には無い、DU 固有の機能です。Tachi 1 台と Kunai 2 台(いずれも AVR DU)で、外部基準に対して周波数を連続測定した結果をまとめます。

| 状態 | 周波数の精度(実測) |
|------|---------------------|
| USB 接続中 | **±0.12% 以内**。温度が変わっても追従して維持 |
| USB 接続後に切断 | 切断時点の補正値を保持。以後は温度に応じてずれる |
| USB に一度も接続せずに起動 | 工場校正のみ。実測 4 台で −0.44〜+0.08%(データシート上の保証は ±2%) |

- **運用中も追従します。** 補正は列挙時だけでなく、USB 接続中は常に働きます。周波数が約 ±0.1% の不感帯を外れると 1 段ずつ戻します。加熱から保冷剤による冷却まで、ダイ温度センサーの読みで最大約 15 °C の幅の温度変化を与えた間、周波数は 3 台とも ±0.12% 以内に保たれました(USB フルスピードの許容は ±0.25%)。
- **補正は段差で行われます。** 1 段は約 0.07%(上げ)/約 0.04%(下げ)で、USB 接続中はこの大きさの周波数の段差が随時起こり得ます。`millis()` や UART には影響しない大きさですが、周波数の短期安定性そのものが必要な用途では考慮してください。
- **USB 非接続時は追従しません。** 内蔵発振器は温度が上がるほど速くなり、その変化は約 +0.015〜+0.035%/°C でした(センサー読みの相対変化から算出)。UART の許容(概ね ±2%)に対しては十分な精度ですが、周波数の絶対精度が要る用途には向きません。
- 温度はダイ温度センサー(`tempCRead()`)で読んだ値の**相対変化**として扱っています。このセンサーは絶対値で ±10 °C 程度の誤差があるためです([Ref_Analog.md](Ref_Analog.md) の `tempCRead()` の項を参照)。
- スケッチから `CLKCTRL.OSCHFTUNE` を手動で書く場合は、データシートと異なる実機の挙動に注意してください([Ref_Errata.md](Ref_Errata.md) の「(DU) OSCHFTUNE」の項を参照)。コアはこのレジスタを書きません。
...
初回のみ、または USB ブートローダを書き込み直す場合は、UPDI プログラマ(PICkit 4/5、Atmel-ICE、SerialUPDI 等)を UPDI パッドに接続して書き込みます。書き込み前に次を確認してください。

1. **JP1 をブリッジする。** 出荷状態は 5V / 3.3V の両側とも開放で、ブリッジするまで MCU の VDD には電源が来ません(電源 LED は LDO 出力側なので点灯します)。
2. **裏面パッドの実配置を確認する。** USB 端子裏のパッドは XIAO の SWDIO / SWCLK 位置が VCC / UPDI に置き換わっています。
   **rev0.2 正誤表:** この 2 パッドはシルク印刷が実際の回路と**逆**です。シルクで `VCC` と書かれた側が UPDI(PF7)、`UPDI` と書かれた側が VCC です。
   PF7 は工場出荷時に内部プルアップが有効なので、USB 給電した状態で UPDI 側パッドの電圧が VDD 付近になっているかを測れば見分けられます。
3. UPDI 通信には GND と UPDI の 2 本で足ります。基板は USB から給電し、プログラマの VCC 出力は接続しないでください。

<sub>開発用 VID/PID は pid.codes のテスト範囲(アプリ `0x1209:0x000A` / ブートローダ `0x1209:0x0009`)を使用しています。</sub>
