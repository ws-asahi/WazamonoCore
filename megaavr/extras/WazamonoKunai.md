...
- The 48 MHz USB clock (CLK_USB) is generated by the internal PLL48M. While USB is connected, OSCHF tracks the USB SOF (a 1 ms signal timed by the host's crystal), so USB works to specification without a crystal (see "Measured auto-tune behaviour" below).
