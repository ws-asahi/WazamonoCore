...
### Logic
[CustomLogic Readme](../libraries/CustomLogic/README.md) The CCL (Configurable Custom Logic) is a small block of hardware look-up tables that evaluate logic with zero CPU involvement. The CustomLogic library exposes it through pre-instantiated units on fixed, board-appropriate pins: pick a gate (AND/OR/XOR/NAND/NOR/XNOR/NOT) or supply any 3-input truth table, read the result, or get attachInterrupt()-style callbacks when the output changes. Advanced CCL features (event inputs, feedback, sequential logic) can be driven through direct register access - see the CCL chapter of the data sheet.

### AnalogComp
[AnalogComp Readme](../libraries/AnalogComp/README.md) The DU-series has an on-chip analog comparator (AC0). The AnalogComp library exposes it through a single pre-instantiated object with fixed, board-appropriate default pins: compare two voltages, compare against an internal reference (DACREF), add hysteresis, drive the result onto PA7, or get attachInterrupt()-style callbacks when the inputs cross.
